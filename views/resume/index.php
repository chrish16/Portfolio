		<section class="content-row content-row--left-align">
			<div class="anchor">
				<h2 class="resume-heading">Work Experience</h2>

				<div class="resume-entry">
					<span class="resume-date">June 2013 &mdash; August 2014</span>
					<strong>Web Developer</strong> at <strong>Appliances Connection</strong>
					<span class="resume-location">(Brooklyn, New York)</span>
					<p class="resume-desc">Worked with the marketing department to maintain the website, promote sales, and design then implement various internal and consumer-facing web projects.</p>
				</div>

				<div class="resume-entry">
					<span class="resume-date">January 2009 &mdash; December 2012</span>
					<strong>Assistant</strong> for <strong>Linda Ball</strong>
					<span class="resume-location">(Hollywood, Florida)</span>
					<p class="resume-desc">Researched and typed documents, made charts and graphs, scanned and organized anything from legal paperwork to receipts and provided general computer help/maintenance and hardware setup.</p>
				</div>

				<h2 class="resume-heading">Education</h2>

				<div class="resume-entry">
					<span class="resume-date">January 2012 &mdash;</span>
					<strong>University of Central Florida</strong>
					<span class="resume-location">(Orlando, Florida)</span>
					<p class="resume-desc">B.S. Computer Science (incomplete)</p>
					<ul class="resume-list">
						<li>GPA: 4.0 &mdash; President's Honor Roll/Dean's List</li>
					</ul>	
				</div>

				<div class="resume-entry">
					<span class="resume-date">August 2007 &mdash; June 2011</span>
					<strong>South Broward High School</strong>
					<span class="resume-location">(Hollywood, Florida)</span>
					<ul class="resume-list">
						<li>GPA: 3.5 &mdash; Honor Roll/AP student</li>
						<li>Graduated with over 250 community service hours</li>
						<li>Drum major of marching band for 2 years and band member for 4 years</li>
					</ul>
				</div>

				<h2 class="resume-heading">Skills</h2>

				<div class="resume-entry">
					<strong>Software/Technologies:</strong>
					<ul class="resume-list">
						<li>Operating Systems: Windows, Mac OS X</li>
						<li>Languages/Frameworks: HTML, CSS, JavaScript, jQuery, PHP, MySQL</li>
						<li>Other Tools: Apache, Git, Dreamweaver (<a href="/downloads/DW_CS4.pdf">2x</a> <a href="/downloads/DW_CS5.pdf">certified</a>), Photoshop (<a href="/downloads/PS_CS4.pdf">2x</a> <a href="/downloads/PS_CS5.pdf">certified</a>), Word, Excel, PowerPoint, Publisher
						<li>Keywords/Competencies: web design, front end, back end, web development, responsive design, cross-browser compatibility, html emails, eCommerce, SEO, branding, social media, Microsoft Office, word processing, spreadsheets, presentations, data entry</li>
					</ul>
				</div>

				<div class="resume-entry">
					<strong>Miscellaneous:</strong>
					<ul class="resume-list">
						<li>Saxophonist and pianist</li>
					</ul>
				</div>

				<a href="/downloads/resume.pdf" class="resume-button">Download PDF Version &rsaquo;</a>
			</div>
		</section>